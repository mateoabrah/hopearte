...
<div class="container mx-auto px-4 py-8 mt-16">
    <div class="bg-[#2E2E2E] rounded-lg p-8 shadow-lg max-w-4xl mx-auto">
        <h1 class="text-3xl font-bold text-[#FFD700] mb-6">Editar cervecería: {{ $brewery->name }}</h1>
        
        <form action="{{ route('breweries.update', $brewery) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="5" required
                    class="mt-1 w-full rounded-md border border-gray-700 bg-[#2E2E2E] p-2 text-white focus:border-[#FFD700] focus:outline-none">{{ old('description', $brewery->description) }}</textarea>
                @error('description')
                    <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                @if($brewery->image)
                    <div class="my-2">
                        ...
                        <img src="{{ asset('storage/' . $brewery->image) }}" alt="{{ $brewery->name }}" class="h-40 object-cover rounded-md">
                    </div>
                @endif
                
                <input type="file" id="image" name="image" accept="image/*"
                    class="mt-1 w-full rounded-md border border-gray-700 bg-[#2E2E2E] p-2 text-white focus:border-[#FFD700] focus:outline-none">
                <p class="text-xs text-gray-400 mt-1">Formato recomendado: JPG, PNG. Tamaño máximo: 2MB. Deja en blanco para mantener la imagen actual.</p>
                @error('image')
                    <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM cargado, inicializando mapa de edición...');
    ...
    function initMap() {
        try {
            ...
            console.log('Leaflet disponible, creando mapa de edición...');
            
            // Centro y zoom basados en si hay coordenadas existentes
            ...
            // Si tenemos coordenadas de la cervecería, colocar marcador inicial
