...
<style>
    #map {
        ...
        position: relative;
        z-index: 1; /* Aseguramos un z-index bajo */
    }
    .leaflet-map-pane,
    .leaflet-tile,
    .leaflet-marker-icon,
    ...
    .leaflet-layer {
        position: absolute;
        z-index: 1; /* Valor más bajo que la navegación */
    }
...
<div class="container mx-auto px-4 py-6 mt-12">
    <div class="bg-[#2E2E2E] rounded-lg p-6 shadow-md max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-[#FFD700]">Crear nueva cervecería</h1>
        ...
        <form action="{{ route('breweries.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <div id="name-error" class="mt-1 text-xs text-red-500 hidden">Este nombre de cervecería ya existe. Por favor, elige otro nombre.</div>
                        @error('name')
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3" required
                            class="w-full rounded-md border border-gray-700 bg-[#2E2E2E] p-2 text-white focus:border-[#FFD700] focus:outline-none">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(function() {
        ...
        // Crear mapa con vista de España por defecto
...
<script>
// Validación de nombre único para cervecería
document.addEventListener('DOMContentLoaded', function() {
    ...
    function validateBreweryName() {
        const breweryName = nameInput.value.trim();
        ...
        fetch(`${window.location.origin}/check-brewery-name?name=${encodeURIComponent(breweryName)}`)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                ...
                if (data.exists) {
                    nameError.textContent = "Este nombre de cervecería ya existe. Por favor, elige otro nombre.";
    ...
    nameInput.addEventListener('input', function() {
        ...
        nameTimer = setTimeout(validateBreweryName, 500);
    });
    
    // Validación del formulario antes de enviar
    breweryForm.addEventListener('submit', function(e) {
        const breweryName = nameInput.value.trim();
        
        // Si el nombre está vacío
        if (breweryName === '') {
            e.preventDefault();
            nameError.textContent = "El nombre de la cervecería es obligatorio.";
            nameError.classList.remove('hidden');
            nameInput.classList.add('border-red-500');
            nameInput.focus();
            return false;
    ...
    // Validar al cargar la página si hay un valor
