<nav class="bg-[#1A1A1A] text-white p-4 fixed top-0 left-0 w-full z-50 shadow-lg transition-all duration-300 ease-in-out">
    <div class="max-w-7xl mx-auto flex justify-between items-center">
                ...
                @auth
                    <div class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-48 bg-[#3A3A3A] rounded-md shadow-lg py-1 z-50">
                            <a href="{{ route('dashboard') }}" class="block px-4 py-2 text-sm text-white hover:bg-[#444444]">
                                Panel Principal
                            </a>
        ...
        <div class="md:hidden" x-data="{ navOpen: false }">
            ...
            <div class="absolute w-full left-0 bg-[#2E2E2E] shadow-lg mt-2" x-show="navOpen" @click.away="navOpen = false">
                <ul class="flex flex-col space-y-2 p-4">
                    ...
                    @auth
                        ...
                        @if(auth()->user()->role === 'company' || auth()->user()->role === 'admin')
                            <li>
                                <a href="{{ route('my_breweries') }}" class="block py-2 text-white hover:text-[#FFD700]">Mis Cervecerías</a>
                            </li>
                        @endif
                        <li>
                            <a href="{{ route('dashboard') }}" class="block py-2 text-white hover:text-[#FFD700]">Panel Principal</a>
                        </li>
                        <li>
                            <a href="{{ route('profile.edit') }}" class="block py-2 text-white hover:text-[#FFD700]">Editar Perfil</a>
                        </li>
                        <li>
                            <a href="{{ route('user.beer_favorites') }}" class="block py-2 text-white hover:text-[#FFD700]">Cervezas Favoritas</a>
                        </li>
                        <li>
                            <a href="{{ route('user.brewery_favorites') }}" class="block py-2 text-white hover:text-[#FFD700]">Cervecerías Favoritas</a>
                        </li>
                        <li>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
